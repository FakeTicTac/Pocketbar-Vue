...
<template>
  ...
  <div class="d-flex">
    ...
    <section class="gallery container-fluid">
      ...
      <h1 class="text-center mt-5 mb-4" style="color: white;">Select Drinks And Their Amounts For Cocktail!</h1>

      <!-- Ingredients Cards -->
      <div class="row card-row">
        <div v-for="drink in showDrinks" v-bind:key="drink.id" class="card-data col-lg-3 mb-4">

          <div :class="drink.isSelected ? 'bordrening' : 'card-unactive'" class="card h-100">
             <img v-if="drink.coverImagePath" class="card-img-top" :src="drink.coverImagePath" alt="Card image cap" @click="choose(drink)">
             <img v-else class="card-img-top" src="@/assets/icons/no-image.png" alt="Card image cap" @click="choose(drink)">
            <div class="card-body">

              <!-- Ingredient Data -->
              <h5 class="card-title fs-4 text-center">{{ drink.name["en-GB"] }}</h5>
              <hr class="hr-top" />
              <p class="card-text">{{ drink.description !== undefined ? drink.description["en-GB"] : "Drink Type Doesn't Have Description." }}</p>

              <div v-if="drink.isSelected">

                  <div class="mb-4">
                    <input :id="`${drink.id}amount`" @input="inputBinder(drink, $event)" type="number" class="form__field" placeholder="Ingredient Amount" />
                  </div>
                                      <!-- Drink Type Field -->
                  <select @input="selectBinder(drink, $event)" class="form-select mt-4">
...
<script lang="ts">
  ...
  import { DrinkInCocktailService } from "@/services/DrinkInCocktailService";
  ...
  import type { IDrinkInCocktail } from "@/domain/IDrinkInCocktail";
    ...
    export default class WorkshopDrink extends Vue {
      ...
      * Drink Related Service Connection Definition.
      */
      drinkInCocktailService = new DrinkInCocktailService();
      ...
      /**
       * Defines Storage For Chosen Drinks In Cocktails.
       */
      ChosenDrinks: IDrinkInCocktail[] = [];
      ...
      */
      async mounted(): Promise<void> {
        ...
        // Load To Storage  
        let drinkResult = await this.drinkService.getAll();
        ...
        this.drinkStore.$state.drinks = drinkResult.data!;

        this.amountUnitStore.$state.amountUnits = amountUnitResult.data!;

        this.cocktailStore.$state.cocktails = [cocktailResult.data!]

        this.ChosenDrinks = this.cocktailStore.$state.cocktails[0].drinksInCocktails!


        // Show Already Chosen Ones.
        this.ChosenDrinks.forEach(x => {

          let chosenDrink = this.drinkStore.$state.drinks.find(y => y.id === x.drinkId);

          chosenDrink!.isSelected = true;
      ...
      /**
       * Method Binds Data To Drink In Cocktail Object.
       * @param {IDrink} drink Defines Drink To Be Binded
       * @param {Event} event Defines Event Invoked. 
       */
      inputBinder(drink: IDrink, event: Event) {

          let drinkInCocktail = this.ChosenDrinks.find(x => x.drinkId == drink.id)!;
          drinkInCocktail.amount = Number.parseInt((event.target as HTMLInputElement).value);

      }

      /**
       * Method Binds Data To Drink In Cocktail Object.
       * @param {IDrink} ingredient Defines Drink To Be Binded
       * @param {Event} event Defines Event Invoked. 
       */
      selectBinder(drink: IDrink, event: Event) {

        let drinkInCocktail = this.ChosenDrinks.find(x => x.drinkId == drink.id)!;
        drinkInCocktail.amountUnitId = (event.target as HTMLInputElement).value;

      }

      /**
       * Method Chooses Drink To Be Added Into Cocktail Receipe.
       * @param {IDrink} drink Defines Drink To Be Added.
       */
      choose(drink: IDrink){
        ...
        if (drink.isSelected) {
          ...
          let drinkInCocktail = this.ChosenDrinks.indexOf(this.ChosenDrinks.find(x => x.drinkId == drink.id)!);
          this.ChosenDrinks.splice(drinkInCocktail, 1);
      ...
      async nextStep() {
        ...
        if(!error) {
          ...
          this.ChosenDrinks.forEach(x => {
            ...
            this.drinkInCocktailService.add(x);

          });

        
          this.$router.push({ name: 'workshop-step-page', params: { id: this.id } })
