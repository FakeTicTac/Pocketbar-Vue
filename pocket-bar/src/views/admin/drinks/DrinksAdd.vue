            ...
            <div class="col-12 col-md-8 col-lg-6 col-xl-5">
              <div class="card bg-dark text-white data-box">
                <div class="card-body p-5 text-center">
                  ...
                  <!-- Action -->
                  <h2 class="fw-bold mb-2 text-uppercase">{{ id === undefined ? "Create" : "Modify" }} Drink</h2>

                  <!-- Action Description -->
                  <p class="text-white mt-5 mb-5">Please Enter Drink Information Below!</p>
                  ...
                  <!-- Alcohol By Volume Field -->
                  <div class="form-outline form-white mb-4">
                    <input v-model="alcoholByVolume" type="number" class="form-control form-control-lg" placeholder="Here Goes Alcohol %" />
                  </div>

                  <!-- Descriotion Field -->
                  <textarea  v-model="description" class="form-control" id="textAreaExample" rows="4" style="background: #fff; " placeholder="Describe Your Drink">
                  </textarea>

                  <!-- Drink Type Field -->
                  <select v-model="drinkTypeId" class="form-select mt-4">
                    <option value="" disabled selected>What's Your Drink Type</option>
                    <option v-for="drinkType in drinkTypeStore.$state.drinkTypes" :key="drinkType.id" v-bind:value="drinkType.id" >{{ drinkType.name["en-GB"] }}</option>
                  </select>

                  <!-- Image Field -->
                  <div class="form-outline form-white mt-5 mb-4">
                    <input v-model="coverImageFile" type="text" class="form-control" placeholder="Your Drink Image URL" />
                  </div>
                  ...
                  <button @click="$router.push('/drinks')" class="right-btn btn btn-outline-light btn-lg px-5 mt-5">Back</button>
...
<script lang="ts">
  ...
  import { DrinkTypeService } from "@/services/DrinkTypeService";
    ...
    export default class DrinksAdd extends Vue {
    ...
    */
    ...
    /**
    * Alcohol By Volume Related Description Information.
    */
    alcoholByVolume: number = 0;

    /**
    * Type Of Drink ID Related Description Information.
    */
    drinkTypeId: string = '';
    ...
    */
    async mounted(): Promise<void> {
        ...
        this.drinkTypeStore.$state.drinkTypes = (await this.drinkTypeService.getAll()).data!;

        // ID Key Is Passed To View.
        if (this.id !== undefined) {

          let data = (await this.drinkService.get(this.id as string)).data;

          this.name = data?.name["en-GB"]!;
          this.description = data?.description!["en-GB"]!;
          this.alcoholByVolume = data?.alcoholByVolume!;
          this.drinkTypeId = data?.drinkTypeId!;
    ...
    * @param {Event} event Defines Event Happenned To Lead To This Action. 
    ...
    async create(event: Event) : Promise<void> {
      ...
      var result = await this.drinkService.add({
    ...
    */
    async edit(event: Event): Promise<void> {
        ...
        var result = await this.drinkService.update( this.id, { 
