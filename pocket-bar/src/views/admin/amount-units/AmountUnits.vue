  ...
  <div class="d-flex">
    ...
    <section class="gallery container-fluid">
      <div class="header-section">
        ...
        <div class="row add-card-row d-flex justify-content-center">
          <div class="col-lg-12 add-card-container mb-4">
            ...
            <router-link to="/amount-units-add" class="card card-add-button h-100">
      ...
      <div class="row card-row">
        <div v-for="amountUnit in showAmountUnits" v-bind:key="amountUnit.id" class="card-data col-lg-3 mb-4">

          <div class="card h-100">
            <div class="card-body">

              <!-- Amount Unit Data -->
              <h5 class="card-title fs-4 text-center">{{ amountUnit.name["en-GB"] }}</h5>
              <hr class="hr-top" />
              <p class="card-text" style="margin-bottom: 0px;">Used With Ingredients In Cocktails: {{ amountUnit.usageWithIngredientsCount }}</p>
              <p class="card-text">Used With Drinks In Cocktails: {{ amountUnit.usageWithDrinksCount }}</p>

              <!-- Actions To Modify Data -->
              <div class="card-btn-container text-center" style="width: 90%;">
                <RouterLink :to="{ name: 'amount-units-edit-page', params: { id: amountUnit.id } }" href="" class="btn btn-modify fs-5 btn-sm">Modify</RouterLink>
                <a v-if="amountUnit.usageWithDrinksCount === 0 && amountUnit.usageWithIngredientsCount === 0" v-on:click="deleteClikced($event, amountUnit.id)" href="" class="btn btn-delete fs-5 btn-sm">Delete</a>
              </div>
...
<script lang="ts">
  ...
  import { AmountUnitService } from "@/services/AmountUnitService";
    ...
    export default class AmountUnits extends Vue {
      ...
      */
      ...
      /**
       * DefinesAmount Unit Storage To Be Shown.
       */
      showAmountUnits: IAmountUnit[] = [];
      ...
      /**
        * Need To Load Amount Unit From Backend.
      */
      async mounted(): Promise<void> {

        // Load To Storage
        let result = await this.amountUnitService.getAll();
        ...
        this.amountUnitStore.$state.amountUnits = result.data!;

        // Define To Show Storage.
        this.showAmountUnits = this.amountUnitStore.$state.amountUnits;
      ...
      async deleteClikced(event: Event, id: string | undefined): Promise<void> {
        ...
        let result = await this.amountUnitService.remove(id);

        // Check If Operation Succeded.
        if (result.statusCode !== 204) this.errorDelete = true;

        // Load To Storage
        let resultLoad = await this.amountUnitService.getAll();
        ...
        this.amountUnitStore.$state.amountUnits = resultLoad.data!;

        this.showAmountUnits = this.amountUnitStore.$state.amountUnits.filter(x => x.name["en-GB"].toUpperCase().includes(this.searchName.toUpperCase()));
      ...
      search(searchName: string) : void {
        ...
        this.showAmountUnits = this.amountUnitStore.$state.amountUnits.filter(x => x.name["en-GB"].toUpperCase().includes(searchName.toUpperCase()))
